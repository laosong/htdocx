...
class Settings {
  ...
  static parse(settingsXml) {
    ...
    const dom = new DOMParser().parseFromString(settingsXml, 'text/xml');

    //let nodes = getChildrenByTagName(dom.documentElement, 'w:defaultTabStop');
...
class Relations {
  ...
  getRelByRId(rId) {
    let rel = _.find(this.rels, {Id: rId});
    if (!rel) {
    }
    return rel;
  }
...
class Numbering {
  ...
  static parse(numberingXml) {
    ...
    for (let num of numbering.nums) {
      const abstractNum = _.find(numbering.abstractNums, {id: num.abstractNumId});
      if (!abstractNum) {
  ...
  setNumLevel(numId, ilvl) {
    const num = _.find(this.nums, {id: numId});
    if (!num) {
      return null;
    }
    if (!num.abstractNum) {
    ...
    const level = _.find(num.abstractNum.levels, {ilvl: ilvl});
    if (!level) {
      return null;
    }

    let newNumObj = {number: 1, text: null, indentLeft: 0};
...
class Styles {
  ...
  static parseCharacterStyle(styleElem) {
    //not implement
  }

  static parseTableStyle(styleElem) {
    //not implement
  }

  static parseNumberingStyle(styleElem) {
    //not implement
  }
  ...
  getParagraphStyle(styleId) {
    let paragraphStyle = _.find(this.paragraphStyles, {id: styleId});
    if (!paragraphStyle) {
    }
    return paragraphStyle;
  }
...
export function parseDocx(xmls, files) {
...
export async function createHT(docx, htDocImpl, pConvtHelper) {
  if (!docx.document)
    return null;
