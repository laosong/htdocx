...
function createTextRun(textContent, spanNode) {
  let textRunOpts = {text: textContent};
...
function createSpan(ctx, spanNode) {
  ...
  const startData = {type: 'text', data: spanNode.textContent};
...
function createImg(ctx, imgNode) {
  ...
  let imgBuf = null;
  if (ctx && ctx.images) {
    let findIt = _.find(ctx.images, {url: src});
  ...
  if (cssFloat === 'left' || cssFloat === 'right') {
    ...
    drawingOptions.floating = {
      horizontalPosition: {relative: hpRelative, align: hpAlign},
      verticalPosition: {relative: vpRelative, offset: 100},
...
function createParagraph(ctx, pNode, numbering) {
  ...
  const paragraph = new docx.Paragraph({children: pRuns});
  if (!paragraph) {
  }
...
function createTableCell(ctx, tableCellNode) {
  ...
  if (width) {
    tcOptions.width = {size: pxNum2DXA(width), type: docx.WidthType.DXA};
  }
  ...
  } else if (width && tableCell.properties) {
...
function createTableRow(ctx, tableRowNode) {
  ...
  let trOptions = {children: tableCells};
...
function createTable(ctx, tableNode) {
  ...
  let tableOptions = {rows: tableRows};

  if (width) {
    tableOptions.width = {size: pxNum2DXA(width), type: docx.WidthType.DXA};
  }
...
function createBreak(ctx) {
  ...
  const cell = new docx.TableCell({
    children: [paragraph],
    width: {size: 8932, type: docx.WidthType.DXA},
    borders: {
      top: {style: docx.BorderStyle.NONE, size: 1},
      bottom: {style: docx.BorderStyle.NONE, size: 1},
      left: {style: docx.BorderStyle.NONE, size: 1},
      right: {style: docx.BorderStyle.NONE, size: 1},
    }
  });

  const row = new docx.TableRow({children: [cell]});
 ...
 * @returns {*}
 */
export function createDocx(htDoc, images, maths, options) {
  ...
  htDocx.addSection({children: blocks});
